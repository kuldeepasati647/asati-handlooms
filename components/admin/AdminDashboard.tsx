...
const UserManager: React.FC = () => {
    const { users, createUser, showToast } = useApp();
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (formData.name && formData.email && formData.password && formData.address) {
            createUser(formData);
            setFormData({ name: '', email: '', password: '', address: '' }); // Reset form
        } else {
            ...
            <div className="overflow-x-auto">
                ...
                <table className="w-full text-left">
                    ...
                    <tbody>
                        {users.length > 0 ? (
                            users.map(user => (
                                <tr key={user.id} className="border-b border-indigo/10 hover:bg-indigo/5">
                                    <td className="p-3 font-mono text-sm text-indigo-light">{user.id}</td>
                                    <td className="p-3 font-semibold text-indigo">{user.name}</td>
                                    <td className="p-3 text-indigo-light">{user.email}</td>
                                </tr>
                            ))
                        ) : (
                            <tr><td colSpan={3} className="text-center py-10 text-indigo-light">No users created yet.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
        ...
        <div className="min-h-screen bg-ivory text-indigo pt-28 pb-12 animate-fade-in">
            <div className="container mx-auto px-6">
                ...
                <div className="flex justify-center border-b-2 border-indigo/10 mb-8">
                    ...
                    <TabButton tabName="notifications" notificationCount={orders.length}>Notifications</TabButton>
                </div>
