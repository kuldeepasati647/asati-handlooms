...
const UserManager: React.FC = () => {
  ...
  return (
    <div className="bg-white/60 backdrop-blur-md rounded-xl p-6 border border-beige shadow-lg">
      ...
      <div className="overflow-y-scroll max-h-80 border border-indigo/20 rounded-lg" onScroll={handleScroll}>
          ...
          <tbody>
            {visibleUsers.length > 0 ? (
              visibleUsers.map(user => (
                <tr
                  key={user._id}
                  className="border-b border-indigo/10 hover:bg-indigo/5 cursor-pointer"
                >
                  <td className="p-3 font-mono text-sm text-indigo-light flex items-center gap-2">
                    <span onClick={() => setSelectedUser(user)} className="cursor-pointer">
                      {user.UserId}
                    </span>

                    <button
                      onClick={() => copyToClipboard(user.UserId)}
                      className="text-indigo hover:text-indigo-700 transition"
                      title="Copy User ID"
                    >
                  ...
                  <td className="p-3 font-semibold text-indigo" onClick={() => setSelectedUser(user)}>
                    {user.name}
                  </td>

                  <td className="p-3 text-indigo-light" onClick={() => setSelectedUser(user)}>
                    {user.email}
                  </td>
              ...
              <tr>
                <td colSpan={4} className="text-center py-10 text-indigo-light">
                  No matching users.
                </td>
              </tr>
            )}
